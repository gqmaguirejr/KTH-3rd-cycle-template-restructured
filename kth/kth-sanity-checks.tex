...
\cs_new_protected:Npn \kth@perform@sanity@check_expl3: {
    ...
    % Added checks
    \kth@check@value:nnn {programcode}{\@programcode}{KTHXXX}
    \kth@check@value:nnn {degreeName}{\@degreeName}{XXX}
    \ifinswedish
    \kth@check@value:nnn {subjectArea}{\@subjectArea}{*****Okänt~ämneområde*****}
    \else
    ...
    \kth@check@value:nnn {orcid}{\@orcid}{xxxxx-xxxx-xxxx-xxxx}
    ...
    \kth@check@value:nnn {secondorcid}{\@secondorcid}{xxxxx-xxxx-xxxx-xxxx}
    }
    ...
    %% Should one add checks for ISBN and TRITA series and numbers?
