    ...
    if entry ~= "." and entry ~= ".." then
      ...
      if attrs then
        ...
        if attrs.mode == "directory" then
          outputFile:write(string.format("%-10s %s -- Modified: %s (%d bytes)\n", "Directory:", full_path, formatted_time, file_size))
...
\lstdefinestyle{latexExampleForAuthors}{
    ...
    morekeywords={subtitle, alttitle, altsubtitle, hostcompany, courseCycle,
      ...
      nationalsubjectcategories, todo, subsection}
}
...
\begin{abstract}
This document describes the \LaTeX{} thesis template developed for use at \gls{KTH} for third-cycle (doctoral and licentiate) theses. The template is \textbf{not prescriptive}: not every thesis will include every part illustrated here, as formats range from monographs to compilation theses, and structural requirements vary across subjects and research methodologies. 

Configured for use with \LuaLaTeX{}, the template simplifies the authoring process by centralizing thesis metadata in a configuration file (\fname{custom\_configuration.tex}) and automating the generation of covers, title pages, divider tabs (if there are included publications), and metadata for \gls{DiVA} and \gls{LADOK} reporting. This guide provides detailed instructions for configuring, structuring, and writing the thesis, complementing the concise two-page \fname{Quick\_Start\_Guide.tex} with in-depth technical advice and guidance on the defense and publication process.

Fundamental to the design of this template are several key factors:
\begin{itemize}
    \item Helping students produce a high-quality thesis,
    \item Helping students be successful in their thesis defense, and
...
One of the hardest problems an author faces is getting started writing, \ie the blank sheet of paper --- empty file barrier. The template (located in the file \fname{examplethesis.tex}) provides a \mbox{non-blank} starting point, helping avoid the blank-page barrier. Additionally, the template provides some initial structure, basically, an \gls{IMRAD} structure, so that there are hints of where to place material. Moreover, there are places (and notes) about material that the author should consider adding; for example, the \enquote{required reflections} section in the final chapter.

\Needspace*{7\baselineskip}
The template also provides examples of common content types, so you can easily find examples of how to include a figure, table, code listing, \etc. These examples are not exhaustive. Quite often, the author will want or need to learn new \LaTeX{} commands in the course of writing their thesis. The need to learn additional \LaTeX{} commands depends largely on how much the author wants to control the thesis details and on their interest (or lack thereof) in \LaTeX{} programming.

As an author, the first step is to make your own copy of the template.
If you are using Overleaf and have opened the template from a \gls{URL}, click \textbf{File} in the upper left-hand corner. Then select \textbf{Make a copy}. Use an informative project name; for example, include your name in the project title so supervisors can readily identify it. If you are using GitHub, you can clone the project from \url{https://github.com/gqmaguirejr/KTH-3rd-cycle-template-restructured}. You can also set up Overleaf to synchronize with your GitHub repository (using \enquote{Integrations \textrightarrow{} GitHub}).

The template \textbf{only} works with \LuaLaTeX. 
If you are using Overleaf, it is easy to select the compiler (\ie \TeX{} engine) by using the drop-down menu \enquote{File \textrightarrow{} Settings \textrightarrow{} Compiler}. You must configure the \LaTeX{} engine and set the main document to \fname{examplethesis.tex}.

Overleaf makes many of the fonts you might use available. If you are running locally, you must ensure you have installed the fonts you need.

Before configuring the template, decide whether you will write a monograph or a compilation thesis. \Cref{sec:cycle3BackgroundOfThesisAndDefense} provides some background about theses and the defense process.

Next, ensure the information about you, your supervisor(s), and the degree is correct in \fname{custom\_configuration.tex}; this information uses various macros, as described in \Cref{sec:configuringCustomConfig} and \Cref{sec:cycle3authorConfigs}. 

With the administrative configuration completed, you are ready to begin writing --- see \Cref{sec:cycle3startingToWrite}.

\Needspace*{9\baselineskip}
Note that if you are using Overleaf:
\begin{description}[style=nextline, labelwidth=\widthof{\textbf{Timeouts}}, leftmargin=!]
...
If you log in to Overleaf with your \gls{KTH} account, you get a version of Overleaf that lets you turn on \enquote{Track changes} which is very useful (particularly if you have invited your supervisors to join your project). It also gives you a larger time budget\footnote{Giving you $\sim$\qty{240}{\second} of wall-clock time, rather than \qty{100}{\second}.} to compile (which can be useful if you have a lot of TikZ figures or other things that take a lot of time for the \LaTeX{} engine to render).

\item[Timeouts] If you have just started a new session (or started working after a long idle time), you may experience a timeout. This commonly happens because the fonts that you are (or were) using are no longer in the font cache. If you think this is the case, then add an \enquote{early} \texttt{\textbackslash end\{document\}} early in your document and compile again. Once the fonts are cached, you can comment out this early \texttt{\textbackslash end\{document\}}. For a better solution, see the document \fname{Saving\_and\_restoring\_font\_cache.tex}; note that this solution is particularly useful when you are using multiple large fonts.

Another cause for timeouts is large images or large amounts of TikZ. One temporary way to handle this is to set the \texttt{draft} option in your document class (this reserves space for figures without loading or rendering raster graphics). A longer-term solution for TikZ is to compile these into images in another document and then include the image.
\end{description}

\section{Configuring custom\_configuration.tex file}
\label{sec:configuringCustomConfig}

The template is highly configurable, letting you enter data once and reuse it. Many macros have been defined. By setting values via these macros, you can set them once and reuse them in many places, reducing the work of updating them while ensuring consistency. Another reason for many of the macros is to collect the information that is needed to report the thesis, \ie to report the title(s), abstracts, keywords, and thesis in \gls{DiVA}, create an errata document (if needed), and, after the defense, to report the title(s) and grade in \gls{LADOK}.
For some further (technical) information about this, see \Cref{sec:cycle3authorMacros}.

\Needspace*{5\baselineskip}
The \fname{custom\_configuration.tex} is designed to capture some overall information about the thesis that is likely to be known (at least in draft form) at the time you are starting your thesis. In fact, much of the material (such as your information and your supervisors' information) will be known since you started your third-cycle studies!
...
These macros also save the data for later use with \gls{DiVA} and \gls{LADOK}. This saved data can also be used to generate announcements for the defense and other purposes. All of this enables you to enter the data \textbf{once} while ensuring that all subsequent uses will be consistent.

Although you do not need to fill in an alternative title (alttitle) and an optional alternative subtitle (altsubtitle) until later, it seems reasonable to locate them together with the title and subtitle information. You will likely revise these fields over time.

\warningExpl{$☡$ The title will be set on the cover and the title page in Figtree Bold and Semibold. However, Figtree does not include Greek characters (and many other characters and symbols); thus, if you want to include such characters, you have to do a little more work, see \Cref{sec:advancedIssuesConcerningCharacters}.}

\subsection{Author macros}
\label{sec:cycle3authorMacros}
The template assumes there can be only one author, or at most two. Generally, third-cycle theses only have one author\footnote{It is \textit{extremely rare} for there to be a second author for a third-cycle thesis, but if there is, see \Cref{sec:READMEauthorSecondAuthor}.}.

For the author, there are a number of macros defined to store information about the author, so that it can later be used in multiple places --- for example, the \gls{KTH} cover (produced with \texttt{\textbackslash kthcover}), the title page (produced with \texttt{\textbackslash titlepage}, the \enquote{For DIVA} section\footnote{Note that the actual section name has \gls{DiVA} set in all caps - which hopefully should \textbf{not} occur in the thesis! If the string \gls{DiVA} set in all caps does have to appear, then the section heading should be preceded by four euro signs and followed by four more euro signs (as is done in this document).} at the end of the thesis (produced with \linebreak[4]
{\small \texttt{\textbackslash divainfo\{pg:lastPageofPreface\}\{pg:lastPageofMainmatter\}}}), \linebreak[4]and possibly a \gls{JSON} file named \fname{fordiva.json} produced as a byproduct of the \texttt{\textbackslash divainfo}. Additionally, the author's name will be included in the \gls{DiVA} entry, in the \gls{LADOK} grade entry, and on the diploma.

The author-related macros are:
\begin{description}[style=nextline, leftmargin=!, labelwidth =\widthof{\texttt{\textbackslash authorsFirstname\{\}}}]
\item [\texttt{\textbackslash authorsLastname\{\}}] the last name of the author\footnote{Note that the author's name can include a suffix, such as \enquote{, Jr.} or \enquote{ Jr.},  set as the author prefers to write their name.}
...
This information will be used when generating the acknowledgments signature, \ie the text at the end of the acknowledgments; it gives the place where the author(s) is/are when writing the acknowledgments, as well as the date and name(s).
...
\item [\texttt{\textbackslash authorCityCountryDate\{\}}] pass into this function the month and year for the acknowledgment. This can be a string such as January 2022 or it can be a \LaTeX{} macro expression, such as:\\ \quad \texttt{\textbackslash MONTH\textbackslash enspace\textbackslash the\textbackslash year}.
\end{description}

\Needspace*{8\baselineskip}
If the author was working at a company, such as \enquote{Företaget AB}, or an organization, such as \enquote{CERN}, they can specify this using one of the following macros.
\begin{lstlisting}[style=latexExampleForAuthors]
...
\warningExpl{A limitation of the current template is that I do not handle multiple companies, as I assume that there is a single host company/organization or employer. However, you can list names within the two text fields (but include only a single \textbackslash hostcompany or a single \textbackslash hostorganization). As of the \gls{DiVA} administrators' meeting of 2022-03-25, if there is a list of names, they should be separated by a semicolon or vertical bar (\ie the `|' character) and the information should be entered in a uniform manner (\ie \enquote{\foreignlanguage{swedish}{på ett enhetligt sätt}}). However, a list of partner names in canonical form is not yet available. Moreover, it was also noted that if the name of the company or organization is confidential, then the field should contain the string \enquote{Confidential} and when the \gls{DiVA} entry is made, the actual name of the company or organization should be entered in the \enquote{Internal} information field (\ie \enquote{\foreignlanguage{swedish}{intern kommentar}}) in \gls{DiVA}. Unfortunately, it is unclear how the \gls{DiVA} administrator would know this information. However, 
...
Several macros are defined to handle information about each of your supervisors. You can edit these entries in the \fname{custom\_configuration.tex} file or generate them using a program.

\Cref{lst:supervisor} shows the case of a program generating the information for a supervisor who has the e-mail address \enquote{maguire@kth.se}. It shows the case for the first supervisor when the supervisor is in the Canvas course 11. Because the supervisor is enrolled in this course, the program can get information about the supervisor based on their e-mail address.
\Cref{lst:supervisorB} shows the case for supervisor `B'.
\Cref{lst:supervisorC} shows the case for an external supervisor `C'. Note that the information about the school and department (in the case of a supervisor within \gls{KTH}) requires the user have a \gls{KTH} \gls{API} key \footnote{However, if you do \textbf{not} have an \gls{API} key, the program cannot output the school or department information. You are, of course, free to write your own program, revise the program, or manually edit the entry in the file. The program is available from \url{https://github.com/gqmaguirejr/Canvas-tools}.}.

\Needspace*{10\baselineskip}
\begin{lstlisting}[language={bash}, breaklines=true, postbreak=\usebox{\lstbreakbox}, caption={Generating \LaTeX~commands for a \gls{KTH} supervisor}, label=lst:supervisor, basicstyle=\footnotesize]
./whoami_for_latex.py maguire@kth.se 11
% If not the first supervisor, then replace supervisorAs with supervisorBs or supervisorCAs as appropriate
\supervisorAsLastname{Maguire}
...
\supervisorAsDepartment{unknown}
\end{lstlisting}



\begin{lstlisting}[language={bash}, breaklines=true, postbreak=\usebox{\lstbreakbox}, caption={Generating \LaTeX~commands for a \gls{KTH} supervisor B}, label=lst:supervisorB, basicstyle=\footnotesize]
./whoami_for_latex.py maguire@kth.se 11 B
\supervisorBsLastname{Maguire}
...
\supervisorBsDepartment{unknown}
\end{lstlisting}

\begin{lstlisting}[language={bash}, breaklines=true, postbreak=\usebox{\lstbreakbox}, caption={Generating \LaTeX~commands for an external supervisor}, label=lst:supervisorC, basicstyle=\footnotesize]
...
Some additional configuration that the author might do is to set the values of the macros related to the course cycle, date of the thesis, degree/exam name, subject area, and if the work for the degree is done external to \gls{KTH} to set the host information (see the file \fname{custom\_configuration.tex}). Consider the snippet below for a doctoral student in Information and Communication Technology (INFKTEKN):
\begin{lstlisting}[style=latexExampleForAuthors]
...
National subject categories are a \textbf{required} field in the \gls{DiVA} record. These categories follow a definition by \foreignlanguage{swedish}{SCB} (nowadays known as \foreignlanguage{swedish}{Statistikmyndigheten} or in English: Statistics Sweden) and HSV (\foreignlanguage{swedish}{Högskoleverket} - nowadays known as  \foreignlanguage{swedish}{Universitetskanslersämbetet (UK-ämbetet)} and \foreignlanguage{swedish}{Universitets- och högskolerådet (UHR)} or in English: Swedish Higher Education Authority and Swedish Council for Higher Education).
While these codes refer to research areas, these codes are also used at \gls{KTH} to indicate the area of the thesis. The Linköping University library advises using 5-digit codes when possible.
...
\Needspace*{7\baselineskip}
You can find examples of the subjects and their codes in \Cref{tab:examplesOfNationalSubjectCategories}. The complete list can be found at \url{https://www.scb.se/contentassets/3a12f556522d4bdc887c4838a37c7ec7/standard-for-svensk-indelning--av-forskningsamnen-2011-uppdaterad-aug-2016.pdf}\\
and\\
\url{https://www.scb.se/contentassets/10054f2ef27c437884e8cde0d38b9cc4/oversattningsnyckel-forskningsamnen.pdf}

\fname{README\_notes/National\_Subject\_Categories.tex} also\linebreak[4] contains the full list. This file can be processed separately by selecting this filename as the main document or by simply clicking on the file in the list of files and then clicking on the \enquote{Recompile} button. The full list is $\sim$16 pages long.

\begin{table}[!ht]
\begin{small}
  \begin{center}
    \caption{Examples of some national subject categories and their codes}
    \label{tab:examplesOfNationalSubjectCategories}
...
The UN's \glspl{SDG} codes are needed for a field in the \gls{DiVA} record.
Some examples of these \glspl{SDG} are shown in Table~\ref{tab:SDGcodes}.
\begin{description}[leftmargin=!, labelwidth =\widthof{\texttt{\textbackslash nationalsubjectcategories\{\}}}]
\item [\texttt{\textbackslash SDGs\{\}}] comma-separated list of \gls{SDG} codes - each a numerical value
\end{description}

\Needspace*{3\baselineskip}
\noindent You should include those that your thesis contributes to. An example of a thesis supporting \glspl{SDG} 8 and 9:
\begin{lstlisting}[style=latexExampleForAuthors]
\SDGs{8, 9}
\end{lstlisting}
...
As with most of the information in the \texttt{custom\_configuration} file, you can revise it as needed, but it is helpful to provide a starting point to keep your focus.
\clearpage
...
The thesis (if a monograph) or the kappa (if a compilation thesis) becomes a public document from the time of the posting (\enquote{nailing up}\footnote{\foreignlanguage{swedish}{In Swedish: spikningen}}) of the thesis in public. At \gls{KTH}, the thesis should \enquote{be made available (posted) in both printed and electronic form, at least three weeks prior to the doctoral defense or licentiate seminar} (see \href{https://www.kth.se/en/biblioteket/publicera-analysera/hantera-publikationer/om-att-spika-avhandling-1.854781}{About posting a thesis}). This period \textbf{excludes} the summer recess and semester breaks.

The printer must send printed copies of the full thesis to the National Library of Sweden and a list of repository libraries distributed around Sweden. Additionally, it is customary to send the thesis to others, such as the department(s) at other universities that might be interested in this thesis. At \gls{KTH}, the public document can be made available via \gls{DiVA}, and a printed copy is \textbf{always} available on request from the Registrar’s Office at KTH (\textit{registrator@kth.se}).

An important thing to understand about the doctoral/licentiate oral disputation (or public defense\footnote{
\ifinswedish
\foreignlanguage{swedish}{8 kap. 13 § högskoleförordningen (1993:100) \enquote{skall disputationer försvaras offentligt}}
\else
Chapter 8, Section 13 of the \textit{Higher Education Ordinance} (1993:100), dissertations shall be defended publicly
\fi
}) is that it is the \textbf{thesis} (published weeks in advance) - plus possible errata (presented \textit {before} the start of the defense)\footnote{For formatting of the errata see \Cref{sec:cycle3Errata}.} - that has to be defended\footnote{The oral defense is really a defense of the \textbf{thesis} and \textbf{not} an \textit{evaluation} of the \textbf{student}.}. At \gls{KTH}, there is \textbf{no} possibility to \textit{revise} a thesis \textit{after} a defense. This is unlike the case in many U.S. universities where the thesis is not published before the oral defense; hence, the thesis can be revised \textit{after} the defense to address issues brought up at the defense.

At \gls{KTH}, the outcome of the defense (as decided by the grading committee)\footnote{The members of the grading committee do not include the supervisors of the student. The members of the grading committee cannot have any conflicts of interest with the student, such as having been a co-author with the student on prior publications, sponsored the student's research, \etc.} is either \textbf{Pass} or \textbf{Fail}\footnote{For further background see, \enquote{Guideline on Third-Cycle Studies}, V-2019-0111 \url{https://intra.kth.se/polopoly_fs/1.1306945.1724056475!/Guideline-on-Third-Cycle-Studies-V-2024-0221\%20\%28translation-of-Swedish-orginal\%29.pdf}, last amended by V2024-0221.}. If the decision is \textbf{Fail}, then the student will need to write a new thesis and defend it, but the previously published thesis remains in \gls{DiVA}, and the previous thesis is still in the national and repository libraries. Note that there is \textbf{no} possibility to simply revise the thesis and have a new defense\footnote{This is unlike the case at Stockholm University, where there is the possibility for a student who fails to address the written deficiencies noted by the opponent and grading committee and then submit (once) a revised thesis --- to address these issues to the satisfaction of the grading committee. If the student fails to do so, then they will need to do a new thesis and have a new defense.}.
...
\begin{enumerate}
    \item \enquote{Do you wish to withdraw your thesis before the defense?}

    \item \enquote{Do you have any errata to announce before the defense?}
\end{enumerate}

If the answer to the first question is \enquote{Yes}, then there is no second question and the defense ends.
If the answer to the first question is \enquote{No} and the answer to the second question is \enquote{Yes}, then the student presents the errata. In simple cases, this might be done orally, but in more complex cases, the student makes a printed version of the errata available (along with the printed thesis) at the time of the defense. These errata formally become part of the thesis that is to be defended. Therefore, the errata should be uploaded as supplementary material for the thesis in \gls{DiVA}, as they help define what the thesis was that was defended.

\subsection{Forms of theses at KTH}
\label{sec:cycle3TypsofTheses}

There are two major forms of third-cycle theses at \gls{KTH}\footnote{While the template includes support for textual \& image artifacts and references to other types of artifacts, I have not fully considered additional types of media, such as video, audio, physical artifacts, $\ldots$, and artistic output.  I apologize in advance that the template might not suit a student in a doctoral program, such as: Architecture, \enquote{Art, Technology and Design}; and others. Constructive suggestions for improving the template are welcome.}:
\begin{enumerate}
    \item Compilation thesis (see \Cref{sec:cycle3CompilationThesis}) and
    \item Monograph (see \Cref{sec:cycle3Monographs}).
\end{enumerate}

A large fraction of third-cycle theses at \gls{KTH} are compilation theses, and most of the rest are monographs.  Therefore, the template should help students using the compilation thesis form.

For example, in 2024, for all of \gls{KTH}, there were about 2{\mystixmathfont \times} as many compilation theses as monographs at the licentiate level and 10{\mystixmathfont \times} as many at the doctoral level. However, there is very high variance between the schools, with some schools having no or few licentiate monographs while EECS had more licentiate monographs than compilation theses and only having 5{\mystixmathfont \times} the number of compilation theses as monographs at the doctoral level (see \Cref{tab:cycle3NumberAndTypeOfTheses}).  \Cref{sec:advancedThesisLengths} also shows the variance between schools.
...
\small
  \begin{center}
    ...
    \toprule
    & & \textbf{Total}& \multicolumn{2}{|c|}{\textbf{Licentiate}}& \multicolumn{2}{|c|}{\textbf{Doctoral}} & \multicolumn{2}{|c|}{\textbf{C/M ratios}}\\
...
Doctoral and licentiate candidates frequently inquire about expected page counts. While there is no institutional upper bound on thesis length, very few third-cycle theses exceed \num{1000} pages, and document length does \emph{not} necessarily correlate with scholarly quality. You should include the material you consider relevant without worrying about page counts. This freedom allows you to structure and place your content to make the reader's task easier. \Cref{sec:advancedThesisLengths} (starting on page \pageref{sec:advancedThesisLengths}) provides an empirical breakdown of thesis lengths across \gls{KTH} schools and types of theses.

\section{Configuration of the template}
\label{sec:cycle3authorConfigs}
The template is highly configurable. Two major mechanisms are used: passing configuration options to the document class, \ie \texttt{\textbackslash documentclass} (\Cref{sec:SettingDocClassOptions}) and setting various values via macros. A third mechanism is via flags, but much of this happens behind the scenes.

Note that in many of the following examples of passing configuration options to the document class, you can pass multiple options separated by commas.
...
\item If you are going to write a compilation thesis, then enable the\linebreak[4] \texttt{includepublications} option, by uncommenting this line. Otherwise, the default is to format as a monograph.
...
You can set the default language to \enquote{English} or \enquote{Swedish} by passing an option to the \texttt{\textbackslash documentclass} command.

The language option \enquote{Swedish} sets the conditional \texttt{\textbackslash ifinswedish} to true.  Among other things, this conditional configures the \gls{KTH} cover and the title page to use the chosen language.

The template is designed to handle a thesis written in English or Swedish\footnote{As of {\DTMsetdatestyle{iso}\DTMdisplaydate{2026}{9}{10}{-1}}, the breakdown by language according to \gls{DiVA} is: English~\num{10118},
Swedish~\num{280},
Spanish~\num{4},
Danish~\num{3} (one of these is actually in English),
German~\num{3},
French~\num{2},
Italian~\num{1},
Norwegian~\num{1},
Polish~\num{1},
Portuguese~\num{1}, and
Russian~\num{1}.}. This template \textbf{only} supports English and Swedish covers (as no other cover format has been defined).
...
\subsection{Bibliography management package}
\label{sec:cycle3BibMgmtpackage}

The template is designed to work with the \BibLaTeX{} package, which is more modern and supports more entry types than \BibTeX{}, such as datasets and software. Additionally, \BibLaTeX{} supports a number of \textit{Non-standard Types}\footnote{Such as: artwork, audio, commentary, image, jurisdiction, legislation, legal, letter, movie, music, performance, review, standard, and video}, see §\,2.1.3 of the documentation for the \BibLaTeX{} package (\textit{The \BibLaTeX{} Package: Programmable Bibliographies and Citations}). \BibLaTeX{} also offers additional citation functionality, such as \texttt{\textbackslash fullcite}. \BibLaTeX{} supports a large number of languages (see \textit{Table 2: Supported Languages} of the package documentation). See also \href{https://www.overleaf.com/learn/latex/Bibliography_management_with_biblatex}{Bibliography management with biblatex}. The template supports many types of \glspl{PID}; see \fname{lib/biblatex-and-friends.tex} for details.
...
\begin{lstlisting}[style=latexExampleForAuthors]
    % Default style (common for engineering theses):
    \usepackage[language=english,
                style=ieee,
                isbn=true,
                maxbibnames=99,       % Print full author lists in bibliography
                maxcitenames=2,       % Keep in-text citations compact (e.g., Yu et al. [10])
                mincitenames=1,       % Truncate to first author + et al. in citations
                giveninits=false,     % Render full given names where available
                refsection=none,
                refsegment=none,
                datamodel=ext-eprint, % Support extended PIDs (Handles, MR, etc.)
                backend=biber]{biblatex}
    
    % Alternatively, an author-year scheme:
    %\usepackage[bibstyle=authoryear, citestyle=authoryear, maxbibnames=99, language=english, backend=biber]{biblatex}
...
\label{sec:cycle3aConfigureForIncludingPublications}
...
I generally recommend that every student start by writing a working abstract; this will help you stay focused (see \Cref{sec:cycle3wrtingFirstAbstract}).
It also helps to think about which acronyms you want to include. If you are going to use an acronym multiple times, you should add it to your set of acronyms; see \Cref{sec:cycle3addingAcronyms}. Also helpful when writing are some predefined macros; see \Cref{sec:cycle3predefine}. Using acronyms and the predefined macros will save you time and help to ensure consistency in your writing.
...
To find where you can start to enter your abstract, look in the \fname{examplethesis.tex} file for the line:
\begin{lstlisting}[style=latexExampleForAuthors]
\generalExpl{Enter your abstract here!}
\end{lstlisting}

The template already includes a lot of information to help you enter text, equations, \etc. in your abstract.
\medskip
...
While you can include figures and tables in an abstract, \first this is relatively rare, and \Second this may cause problems when the abstract is entered into \gls{DiVA}.
\warningExpl{This template and the tools that I have written to help enter the metadata into \gls{DiVA} do \textbf{not} support tables and figures in abstracts.}

I suggest avoiding using defined acronyms in abstracts; instead, spell them out rather than using the glossary commands. This is because the \texttt{glossaries} package (used to support acronyms) does not directly support multiple languages, and I do not understand how to programmatically create plurals of acronyms in Swedish or other languages. Even in an English abstract, it is desirable to avoid using the glossary commands, as this makes subsequent processing of the abstracts harder, since one has to make sure that the list of acronyms and their definitions are provided to any program that will process this \LaTeX{}  source code. For this reason, later versions of this template attach the \fname{acronyms.tex} file.

\subsection{Structure of the abstracts and summaries}
The basic \LaTeX{}  structure for an abstract or summary is shown below (for the case of an English abstract and a Swedish summary \ie \foreignlanguage{swedish}{sammanfattning}):
\begin{lstlisting}[style=latexExampleForAuthors]
\selectlanguage{english}
\begin{abstract}
...
\Needspace*{8\baselineskip}
The three-character code used for the language (such as \enquote{eng} or \enquote{swe}) is the ISO 639-2 Code – specifically the \enquote{B} (bibliographic) variant of these codes --- as these codes are used in the \gls{DiVA} metadata to tag which language is used.

The two abstracts above are special because they used the \texttt{\textbackslash InsertKeywords\{XXXX\}} command. This is done so that the keywords defined in the \fname{custom\_configuration.tex} file can be automatically inserted into the abstracts \textbf{and} so that these keywords are known when processing the \LaTeX{} preface, and thus can be put in the \gls{PDF} document's metadata.

\Needspace*{13\baselineskip}
\subsection{Structured abstracts}
\label{sec:cycle3StructuredAbstracts}
Some fields commonly use structured abstracts. These typically have the following parts:
\begin{verbatim}
...
The abstract needs to be able to stand alone; therefore, you \textbf{cannot} include citations to your references --- as the references are \textbf{not} part of the abstract! It is possible (but very rare) to have footnotes as part of the abstract. However, be aware that often, if the abstract is manually entered in \gls{DiVA}, the footnote might not be entered. In this case, unless your full text is available (\eg via \gls{DiVA}), a reader might not have an easy way to find out what the footnote says.

\subsection{Automated checking of some properties of abstracts}
\label{sec:cycle3AbstractChecking}
This template performs some simple checks on the abstract's contents. For example, the template generates warnings if the author includes a citation or cross-reference in the abstract.
...
\Needspace*{7\baselineskip}
Given these two new macros, we use hooks (provided by the \texttt{etoolbox} package) to save the definition of \textbackslash cite before the abstract and restore it afterward --- as well as to redefine \textbackslash cite to produce a warning if used within the abstract. Note that in the \gls{PDF} output, there will be {\NotoSansFont \textcolor{red}{⁉cite⁉}} at the place (in the \gls{PDF}) where the \textbackslash cite was - to draw the user's attention to the error. The same mechanism applies to several other macros.
...
You may want to define an acronym to help you with your writing, as this can both reduce the amount of typing and help your reader by providing consistent use of acronyms. You can find the acronym definitions in the file \fname{lib/acronyms.tex}. The file contains some examples. I generally sort the lines alphabetically to help you find which acronyms have already been defined and to keep track of those I have added.

\subsection{Some predefined macros to help when writing}
\label{sec:cycle3predefine}

The file \fname{lib/defines.tex} includes some macros that will help you when writing. This includes \textbackslash etc, to give you \enquote{\etc}, \textbackslash eg, \textbackslash ie, and \textbackslash etal.
...
You will likely find parts of the template you do not want/need. One way to deal with this is to delete them; another is to comment them out. Personally, I like to comment things out in case I actually want to read them in the \LaTeX{} source file or uncomment them later. To comment out a portion of the file, simply use the \texttt{comment} environment:
...
At some point, you will no longer want the various README information. You can then remove the \textbf{README\_notes} directory and the files \fname{Quick\_Start\_Guide.tex} and \fname{README\_3rd\_cycle\_author.tex}.\linebreak[4] However, you might find them useful (even if you do \textit{not} read them), as they provide a lot of \LaTeX{} examples.

\section{Considerations while writing}
\label{sec:considerationsWhenWriting}
After you start writing, you will have other issues to address. One of these is what copyright or license you want to use; see \Cref{sec:cycle3copyrightOrCClicense}. Another consideration will be using a variety of fonts. Consistent font use can help your reader and facilitate your writing. However, there are some issues to be concerned with; see \Cref{sec:cycle3useOfFonts}. As you write, you may want to split your document up into pieces to make it easier for you to work with it; see \Cref{sec:cycle3onebigFIlevsincludes}.
...
\Needspace*{7\baselineskip}
The student must decide whether to use a traditional copyright or another form, such as a Creative Commons (CC) license. However, in Sweden, each thesis is a public document; therefore, the student \textit{cannot} prevent the university from providing a printed copy to anyone who asks for one (see \Cref{sec:cycle3BackgroundOfThesisAndDefense}). Therefore, if a student uses the traditional copyright form, the student must give the university permission to distribute printed copies of the thesis. Regarding \gls{DiVA}, the \gls{KTH} Library has a Media Agreement form \footnote{Agreement regarding publication} for this purpose.
...
Note that the lowercase version of the Creative Commons license has to be used in the modifier, \ie one of: \enquote{by}, \enquote{by-nc}, \enquote{by-nd}, \enquote{by-nc-nd}, \enquote{by-sa}, \enquote{by-nc-sa}, or \enquote{zero}. For the list of supported licenses, see the \texttt{doclicense} package documentation.
...
Note that the option \enquote{hyphenation={RaggedRight}} can be used with the configuration of the package to set the license information with a ragged right margin rather than as a filled and justified paragraph.
...
Font choice is a very individual matter and may be affected by the kind of content you are writing, the language you are writing in, and what you want to convey to your reader. However, some points to keep in mind are:
\begin{itemize}
    ...
    \item Many fonts are proprietary; thus, you need to consider whether you have an appropriate license to use them.\warningExpl{Caution, the thesis template \enquote{\foreignlanguage{swedish}{Avhandlingsmall Word}} from
...
What can you do when the fonts you use are missing characters that you need to use? One solution is to use a font that includes the character(s) you want, then use them where you need them. For example, one can access additional symbols with code such as\footnote{Note the use of \texttt{escapechar} in the listing to escape to \LaTeX{} to render the glyphs using \texttt{\textbackslash mystixmathfont}.}:
...
\noindent The above commands enable you to use these unicode characters \textit{without} being in math mode. The commands basically say, if the character is in the current font, then just use it, else replace it with the \gls{STIX}~Two~Math character shown.

\engExpl{The characters above were chosen more or less randomly, except for {\mystixmathfont ♀}, which was included because it occurs in a thesis title (as described in a README file) and dangerous bend (caution Sign) {\mystixmathfont ☡} as it is used in this document and other documents.}

\Needspace*{5\baselineskip}
\warningExpl{The details of working with different fonts and characters are a rather complex area and not for the faint-hearted. However, if you \textbf{really} want to have specific characters, \LuaLaTeX\ has the means to help you realize what you want. For some examples, see the font setup in \texttt{kththesis.cls} and the titles of some of the example publications (see other README files).}

\Needspace*{12\baselineskip}
\subsubsection{Use of fonts on the covers, title page, and book info pages}
\label{sec:cycle3useOfFontsonCovers}

The thesis template \enquote{\foreignlanguage{swedish}{Avhandlingsmall Word}} from
\url{https://www.kth.se/student/studier/examensarbete/avhandlingarochexamensarbeten/mall-for-avhandling-1.458236} does not include the cover, title page, and book info pages, as the assumption is that US-AB will make these pages for a third-cycle thesis. However, for students and their supervisors, it is important to include at least the title and book information pages so they can identify the document and ensure the content is correct. Therefore, I have developed covers, title page, and book information page based on my examination of some existing printed theses or \gls{PDF} files of recent theses. These pages primarily use different font faces from the Figtree font family.

\Cref{sec:configuringTitleSubtitle} on page\,\pageref{sec:configuringTitleSubtitle} discusses issues with including mathematical content or characters not in Figtree in titles, subtitles, and headings.
...
\warningExpl{While it generally is possible to get the glyphs that you want displayed on your cover, title page, and book info pages, there might be trouble getting these into \gls{LADOK} and even with the use of \gls{HTML} code to get them into \gls{DiVA}. For more details, see \Cref{sec:advancedIssuesConcerningCharacters}.}

\subsubsection{Use of fonts within the thesis}
\label{sec:authorUseOfFontsWithinThesis}

The thesis template \enquote{\foreignlanguage{swedish}{Avhandlingsmall Word}} specifies the use of fonts for the preface pages and the body of the thesis. Figtree has been used as a sans serif font for headings. They specified Georgia, a proprietary font, as the serif font for the body.
...
\gls{STIX}~Two~Math font as a math font\footnote{See the \gls{STIX} Fonts project (stixfonts) at \url{https://www.stixfonts.org/}.}. Additionally, TeX Gyre Pagella Math can be used as the math font.

Additionally, the template makes available the \gls{STIX}~Two~Math font as \texttt{\textbackslash mystixmathfont}. This font contains a very large number (more than \num{5200}) of glyphs; see the CTAN\footnote{Comprehensive \TeX\  Archive Network} documentation for the package \enquote{\href{https://ctan.org/pkg/stix2-otf/?lang=en}{\texttt{stix2-otf}} --- OpenType Unicode text and maths fonts}. Moreover, the \gls{STIX} fonts are available under the SIL Open Font License. For additional information, see \href{https://www.overleaf.com/learn/latex/Articles/OpenType-based_math_typesetting%3A_An_introduction_to_the_STIX2_OpenType_fonts}{OpenType-based math typesetting: An introduction to the STIX2~OpenType fonts}.
...
\newfontfamily\mystixmathfontSSa{STIX Two Math}[StylisticSet=1]
\end{lstlisting}
\noindent This defines a new font that utilizes the Stylistic Set 1 <ss01> --- giving you access to roundhand glyphs rather than Chancery Script alphanumeric symbols (often referred to as Mathematical Script or Calligraphic characters). It is also possible to access character variants of OpenType fonts by specifying an option, such as \texttt{[CharacterVariant=3]}. For a discussion of OpenType features, see \url{https://learn.microsoft.com/en-us/typography/opentype/spec/featuretags}.

\subsection{One big thesis file or a master file with includes of the parts}
\label{sec:cycle3onebigFIlevsincludes}

Many students split their thesis into multiple files (such as\linebreak[4] \fname{introduction.tex}, \fname{background.tex}, \fname{method.tex}, \linebreak[4]\fname{what-you-did.tex}, \fname{results-and-analysis.tex},\fname{discussion.tex},\linebreak[4]  \fname{conclusion-and-future-work.tex}) and then include these in their main document (with a series of \textbackslash include\{xxxx\}). There can be a problem with compiling the project in Overleaf, as Overleaf does not always handle the separate files as one document (unless you use \texttt{\textbackslash documentclass[examplethesis.tex]\{subfiles\}} for each of the files and use \texttt{\textbackslash subfile\{\textit{filename}\}} to include them in your main file - these commands tell \LaTeX{}  that a file is part of a larger document and identify the parent document)\footnote{Even then, it may have problems - for example with getting the correct fontspec information for Figtree.}. Although I have had some students do this successfully, and they liked being able to compile just a part of their thesis. I have not used this for the \fname{examplethesis.tex} file\footnote{I thought it was simpler to have it as a single file.}.
...
\item    Additionally, Overleaf has a per-file size limit (\ie how large a file can be) --- again this is per-file [Yes, I have gotten bitten by this when exporting a Jupyter notebook that produced a \LaTeX{} file larger than \qty{50}{\mega\byte}.]
\end{itemize}
...
Ultimately, I think the main issue is how separate the files are and whether they can be worked on as if they were very independent. This generally is true in third-cycle theses, as the chapters tend to be rather independent - typically with one publication as the focus of a chapter.

However, some might see splitting as a matter of taste, or as different ways to organize your writing. So you and your supervisor(s) might want to discuss which choice best suits your purposes.

\section{Including publications}
\label{sec:cycle3IncludingPublications}

If you are including publications in your thesis, they are typically placed at the end of the thesis, and before each publication is a divider page. The divider page has a tab with a label, such as 'Paper A' or perhaps just a capital Roman numeral. These tabs make it easier for your reader to refer to the publication when you reference it in your thesis or defense. It is also helpful to have some of the details about the publication and the permission to use the publication in your thesis on this divider page. Especially when someone else or another entity holds the copyright, the copyright holder often requires certain information on a page before the included publication.
...
Inserting the \gls{PDF} of publications is very simple, as shown in \Cref{lst:InsertingPDFofIncludedPublication}. You can set the scaling of the included pages, specify the page range to include, add a frame around the page, and even include a running heading. The last probably requires enabling fancy headings for the pages. Also note that you need to include \texttt{\textbackslash usepackage\{pdfpages\}}\footnote{This is included by default in \fname{lib/includes.tex}.}. If you want to include an image, you can use \texttt{\textbackslash includegraphics\{\textit{filename}\}} or perhaps scaled as \texttt{\textbackslash includegraphics[scale=0.90]\{\textit{filename}\}}.
\Needspace*{10\baselineskip}
\begin{lstlisting}[style=latexExampleForAuthors, caption={Using \texttt{\textbackslash includepdf\{\}} to include the \gls{PDF} of a publication}, label=lst:InsertingPDFofIncludedPublication]
\cleardoublepage
...
As described in \Cref{sec:cycle3BackgroundOfThesisAndDefense}, the errata have to be announced before the student starts their actual oral defense. I have not found a standard format for this, as a printed document. Looking at a number of errata that are in \gls{DiVA} for theses at \gls{KTH}, most have a title of \enquote{Errata} or \enquote{Errata for doctoral/licentiate thesis} followed by a reference to the thesis. This reference can be as short as the title of the thesis \& the author or as long as a full reference (with ISBN). The key is that it should be possible to identify the relevant thesis from the information at the top of the errata. \textbf{NB} If you use the \fname{errata.tex} file in this project, it will automatically include this information.

\Needspace*{5\baselineskip}
The errata can concern the thesis text and may also include text from published papers. Each item in the errata should identify where the correction should be applied and what the correction is. The identification can include a page number (or page range), section heading, figure/table/equation/$\ldots$ , publication identifier, \etc. The aim is for the reader to identify exactly where the change should be made.

\Needspace*{11\baselineskip}
The corrections can range from spelling errors, missing letters, words, phrases, citations, \glspl{ISBN}, \glspl{DOI}, \glspl{URL}, etc. to errors in the text, equations, tables, figures, listings, corrected citations, \etc. For example, there might have been a change in the name of the volume that a paper appeared in after the thesis was submitted for printing. Corrections to tables, figures, \etc may affect parts of the text, potentially invalidating some of the text. This invalidated text would be reproduced in a struck-out style (using the \LaTeX{} command
...
This section covers a number of advanced topics: Advanced issues concerning characters (\Cref{sec:advancedIssuesConcerningCharacters}), how to handle the rare case of a second author (\Cref{sec:READMEauthorSecondAuthor}), and some details of typical thesis lengths (\Cref{sec:advancedThesisLengths}). These subsections are located here as they are probably only of interest to a small number of authors and to keep the earlier sections shorter.

\subsection{Advanced issues concerning characters}
\label{sec:advancedIssuesConcerningCharacters}
\noindent $☡$ The issue of what characters can be placed in a title (subtitle, keywords, and abstracts) and how to enter them is a bit complex, but you should be able to achieve much of what you want - at the cost of some effort.
Ultimately, the characters that will be placed into \gls{DiVA} and \gls{LADOK} must be unicode, encoded using \gls{UTF}, specifically UTF-8. While \gls{DiVA} utilizes \gls{HTML} for these fields, allowing you to include some markup, \gls{LADOK} does not support any markup.

\noindent $☡$ If you have titles or subtitles that cannot be converted to pure Unicode (such as subscripts with capital letters or other symbols), you should also use the optional commands \texttt{\textbackslash titleInPlainText}, \texttt{\textbackslash subtitleInPlainText}, \texttt{\textbackslash alttitleInPlainText}, and \texttt{\textbackslash altsubtitleInPlainText} in your \fname{custom\_configuration.tex}  \fname{custom\_configuration\_plaintext.tex} files to provide versions of titles and subtitles that can be used with \gls{LADOK} and when requesting an \gls{ISBN}. Compiling \fname{Generate\_titles\_in\_plain\_text.tex} will produce a file named \fname{extra\_plaintext.tex} containing plain text versions of the \mbox{titles} and subtitles (both main and alternate).

As noted earlier the title will be set on the cover and the title page in Figtree Bold and Semibold. However, as Figtree does not include Greek characters (and many other characters and symbols), you have to do a little more work. You can enter UTF-8 characters directly into your \LaTeX{} file, but you cannot use the \TeX{} commands, such as \textbackslash Gamma - as this will produce an error message in the log file:
...
\noindent In the above error message the '{\mystixmathfont ^^^^fffd^^^^fffd}' is actually output as a small box with the digits \enquote{01D6E4} in it. This is the hexadecimal value of the character \enquote{MATHEMATICAL ITALIC CAPITAL GAMMA}. However, you can enter the character {\mystixmathfont ^^^^^^01d792} using the hexadecimal code: 
\textasciicircum\textasciicircum\textasciicircum\textasciicircum\textasciicircum \textasciicircum 01d792 - for a \enquote{MATHEMATICAL SANS-SERIF BOLD ITALIC CAPITAL GAMMA}. You should choose the \enquote{SANS-SERIF BOLD ITALIC} version of the character, since the title is set in Figtree Bold, a sans-serif bold font. In the \fname{defines.tex} file you can find an include of a file of \enquote{Mathematical Alphanumeric Symbols, U+1D400 - U+1D7FF}  unicode characters that have been defined to render the character using the \gls{STIX}~Two~Math font\footnote{The file's name is \fname{unicode\_blocks/U+1D400-U+1D7FF-Mathematical Alphanumeric Symbols.tex}.}.


\Needspace*{2\baselineskip}
Alternatively, if you set your math font to use \gls{STIX}~Two~Math with:
\begin{Verbatim}[breaklines=true]
 \setmathfont{STIX Two Math}[version=normal]
\end{Verbatim}
\noindent and use write $\Gamma$ (using \$\textbackslash Gamma\$) in your title, you will \textbf{not} get a missing character message. This occurs because the math font actually has a \enquote{MATHEMATICAL ITALIC CAPITAL GAMMA} character, but it will be the \textbf{serif} version and not a \textit{bold italic} version - since the \textit{bold italic} version of this character is at a different codepoint in the font.

Note that while the \enquote{MATHEMATICAL ITALIC CAPITAL GAMMA} character will appear in the \gls{PDF}, you will get warnings from \texttt{hyperref} about using the math in the title:
\begin{Verbatim}[breaklines=true]
...
\begin{lstlisting}[basicstyle=\scriptsize,breaklines=true, postbreak=\usebox{\lstbreakbox}]
Package hyperref Warning: Token not allowed in a PDF string (Unicode): removing `\Gamma' on input line 56.
\end{lstlisting}
...
\end{Verbatim}
because \gls{STIX}~Two~Math has no bold face (or semibold face), but only bold face \textit{characters} within the font.

\Needspace*{4\baselineskip}
Also note that when you get a hyperref warning, the character is \textbf{not} put into the metadata in the \gls{PDF} file (such as the title and other properties). However, if you put the character in using \textasciicircum\textasciicircum\textasciicircum\textasciicircum\textasciicircum1d792, then this character appears in the document's metadata, the \texttt{PDF}, and the \fname{fordiva.json} file.

\Needspace*{5\baselineskip}
\textbf{NB} You can insert Greek characters such as Γ, Δ, α, β, γ, \ldots\,, by directly entering them, but if you place these into a title, they will \textit{not} be set in boldface, since they are rendered using the \gls{STIX}~Two~Math font which knows nothing about the boldface being used for the title (and, as noted earlier, this font actually does not have a bold version).
\engExpl{Overleaf and many other programs support unicode, so the easiest thing to do is directly enter the characters that you want and not worry too much about the specific font or font face. In most cases, your reader will be glad to have the character there, rather than missing! However, in some cases, you will want and need to have even a specific variant of a character, such as a script \enquote{H} ($\mathscr{H}$, $\mathcal{H}$ or {\mystixmathfontSSa ^^^^210b}) or a bold script \enquote{H} ({\mystixmathfont ^^^^^^01d4d7} or {\mystixmathfontSSa ^^^^^^01d4d7}) for a Hamiltonian operator or $\mathfrak{H}$ for Hilbert space.}

So what happens if you want to put one of these bold script \enquote{H}'s into your title? Consider the following title:
\begin{flushleft}
...
\end{flushleft}
\noindent The linebreak before the \enquote{or} has been inserted for readability.
...
\Cref{fig:titleWithScriptHsAndPropteries} shows the cover together with the document's properties. Curiously, in this figure it looks like both \enquote{H}s have been rendered as the roundhand script \enquote{H} rather than the first being rendered as a chancery script \enquote{H}.
 
\begin{figure}[!ht]
\centering
\includegraphics[width=1\textwidth]{figures/cover_with_script_Hs_in_title_and_document_properties.png}
\caption{Cover with bold script \enquote{H}s and the document's properties}
\label{fig:titleWithScriptHsAndPropteries}
\end{figure}
\FloatBarrier

\Cref{fig:titleWithScriptHsemacs} shows a similar rendering of the \enquote{H}s when using emacs to look at the contents of the \texttt{fordiva.json} file. As expected, we see the \LaTeX{} commands to render the character in the two different fonts.
\begin{figure}[!ht]
\centering
\includegraphics[width=1\textwidth]{figures/json_version_of_title_with_script_Hs.png}
\caption{The \texttt{fordiva.json} showing the \texttt{title} field as shown when using emacs.}
\label{fig:titleWithScriptHsemacs}
\end{figure}
\FloatBarrier

However, if we cut the title out from the property display and place it into an \gls{HTML} file, as shown in the list below. One gets the browser view as shown in \Cref{fig:titleInBrowser}.
\clearpage

\begin{minted}[breaklines, escapeinside=||, fontsize=\footnotesize]{html}
<!DOCTYPE html>
<html>
<head>
...
\Needspace*{10\baselineskip}
If the second author is not in the same place as the first author, then add the relevant information using the macros below.  This information will be used when generating the acknowledgments signature.
\begin{description}[leftmargin=!, labelwidth =\widthof{\texttt{\textbackslash secondAuthorsFirstname\{\}}}]
...
\item [\texttt{\textbackslash secondAuthorCityCountryDate\{\textbackslash MONTH\textbackslash enspace\textbackslash the\textbackslash year\}}]  pass into this function the month and year for the acknowledgment
\end{description}
...
\subsection{Typical thesis lengths}
\label{sec:advancedThesisLengths}
The \href{https://www.kth.se/polopoly_fs/1.982602.1588861401!/Nationalekonomi%20ASP%20fastst%C3%A4lld%202017-02-16.pdf}{\enquote{\foreignlanguage{swedish}{Studieplan för ämne på forskarnivå: Nationalekonomi}}} 2017-02-16, dnr: V-2017-0040 says in §5, that a monograph is typically
\qtyrange[range-units=single,range-phrase=..]{80000}{100000}{words} \ie \qtyrange[range-units=single,range-phrase=..]{220}{260}{pages} while a kappa is \qtyrange[range-units=single,range-phrase=..]{30}{50}{pages}. Some statistics about third-cycle thesis lengths (during 2022 to 12 September 2026) are shown in \Cref{tab:thesesxLengths2022toSept2026}. Note that this table and \Cref{fig:ABETlen,fig:CBHTlen,fig:EECSTlen,fig:ITMTlen,fig:SCITlen} are based upon the total number of pages, \ie, the sum of the number of preface, body, and appendices pages. This data is based on \gls{DiVA} exporting a \gls{MODS}~\cite{SwePubMODS2019} file where the \gls{DiVA} public type codes ('publicationTypeCode') for third-cycle theses are: 'comprehensiveDoctoralThesis',
...
\begin{table}[!ht]
  \begin{center}
    \caption{Thesis lengths 2022 to 11 September 2026}
    \label{tab:thesesxLengths2022toSept2026}
...
ABE&comprehensive Doctoral&142&19&576&87.92&80.50&54.11\\
ABE&monograph Doctoral&32&99&781&311.34&270.50&138.45\\
ABE&comprehensive Licentiate&85&21&131&52.98&48.00&17.95\\
ABE&monograph Licentiate&9&27&271&154.33&142.00&78.09\\
CBH&comprehensive Doctoral&302&39&176&79.94&76.00&21.93\\
CBH&monograph Doctoral&1&128&128&128.00&128.00\\
CBH&comprehensive Licentiate&16&25&128&58.75&58.50&22.49\\
EECS&comprehensive Doctoral&341&30&341&97.40&85.00&45.17\\
EECS&monograph Doctoral&83&102&338&192.93&186.00&49.28\\
EECS&comprehensive Licentiate&36&36&197&66.42&59.50&29.76\\
EECS&monograph Licentiate&41&71&185&114.46&105.00&31.59\\
ITM&comprehensive Doctoral&190&39&379&126.96&111.50&56.46\\
ITM&monograph Doctoral&4&76&270&180.00&187.00&82.39\\
ITM&comprehensive Licentiate&21&44&169&86.38&79.00&37.30\\
ITM&monograph Licentiate&4&44&120&82.25&82.50&35.26\\
SCI&comprehensive Doctoral&237&22&525&108.59&88.00&70.51\\
SCI&monograph Doctoral&5&77&277&165.00&152.00&84.53\\
SCI&comprehensive Licentiate&33&24&203&63.52&48.00&39.23\\
...
\begin{subfigure}[T]{0.5\textwidth}
\centering
\includegraphics[width=1\textwidth]{figures/thesis_statistics_plots/CBH-monograph Doctoral.png}
\caption{ABE monograph Doctoral}
  \label{fig:CBHMD}
\end{subfigure}
\hfill
\begin{subfigure}[T]{0.5\textwidth}
\centering
%\includegraphics[width=1\textwidth]{figures/thesis_statistics_plots/CBH-monograph Licentiate.png}
%\caption{ABE monograph Licentiate}
%  \label{fig:CBHML}
\end{subfigure}\\
\caption[CBH third-cycle thesis length]{CBH third-cycle thesis length  --- Note that there are no monograph Licentiate theses and only one monograph Doctoral thesis.}
  \label{fig:CBHTlen}
\end{figure}
\FloatBarrier
...
\caption{EECS monograph Licentiate}
  \label{fig:EECSML}
\end{subfigure}\\
\caption[EECS third-cycle thesis length]{EECS third-cycle thesis length --- Note that there are a fair number of monographs with a median length of ~105 (licentiate) and ~186 (doctoral) pages.}
  \label{fig:EECSTlen}
\end{figure}
\FloatBarrier
...
\label{sec:furtherDocumentation}
If you have more detailed questions about the underlying template itself, see the \fname{README\_notes/README\_notes.tex} file.

If you have even more detailed questions about the third-cycle template, see the \fname{README\_for\_3rd\_cycle\_version.tex} file.
