...
%% KTH 3rd cycle template restructured repository is at https://github.com/gqmaguirejr/KTH-3rd-cycle-template-restructured
%%
...
%
%EnableBosnianAbstract,      % include Bosnian abstract
%
...
%
%EnableHebrewFont,           % enable Hebrew font support
%EnableHebrewAbstract,       % include Hebrew abstract
%
%EnableDevanagariFont,       % enable Devanagari font support
%EnableHindiAbstract,        % include Hindi abstract
%
%EnableHungarianAbstract,    % include Hungarian abstract
%EnableIcelandicAbstract,    % include Icelandic abstract
%EnableItalianAbstract,      % include Italian abstract
%
%EnableJapaneseFont,         % enable Japanese font support 
%EnableJapaneseAbstract,     % include Japanese abstract
%
%EnableLatinAbstract,         % include Latin abstract
%EnableLatvianAbstract,       % include Latvian abstract
%EnableMeänkieliAbstract,     % include Meänkieli abstract
%EnableNorthernsamiAbstract,  % include Northernsami abstract
%EnableNorwegianAbstract,     % include Norwegian abstract
%EnablePolishAbstract,        % include Polish abstract
%EnablePortugueseAbstract,    % include Portuguese abstract
%EnableRomanianAbstract,      % include Romanian abstract
%
%EnableCyrillicFont,        % enable Cyrillic font support
%EnableRussianAbstract,     % include Russian abstract (this will enable Cyrillic font support if it is not already enabled)
%
%EnableSerbianAbstract,		% include Serbian abstract
%
%EnableSlovakAbstract,		% include Slovak abstract
%EnableSouthsamiAbstract,	% include Southsami abstract
%EnableSpanishAbstract,		% include Spanish abstract
%EnableSwahiliAbstract,		% include Swahili abstract
%EnableTurkishAbstract,		% include Turkish abstract
%EnableUkrainianAbstract,	% include Ukrainian abstract
%
%EnableVietnameseFont,       % enable Vietnamese font support
%EnableVietnameseAbstract,   % include Vietnamese abstract
%
%EnableYiddishAbstract,      % include Yiddish abstract
%
]{kththesis}
...
% Set up babel for the main and secondary languages.
% Babel will automatically add additional languages as they are used.
\ifinswedish
...
% The following is needed in conjunction with generating the DiVA data with abstracts and keywords
...
%\usepackage[acronym, style=super, section=chapter, nonumberlist, nomain,
%nopostdot, notranslate]{glossaries}

\usepackage[acronym, style=super, section=chapter, toc=false, nonumberlist, nomain, nopostdot=true, notranslate]{glossaries-extra}
\ifinswedish
    %\usepackage{glossaries-swedish}
\fi

% glossaries automatically computes the width as \linewidth −4\tabcolsep − W, where W is the width of the widest name
...
% this file could have been created using https://www.overleaf.com/read/tcbkkbjqnkpr#ecb9da
...
% To have a Creative Commons (CC) license and logos use the doclicense package
...
\else
    ...
    % To be able to cite single publications
    % The multibib package is used to create a category for each publication,
    % then these separate bibliographies can be printed
    \usepackage[labeled]{multibib}
    % Define several bibliographies
    % One for each of the publications that you want to include
    \newcites{pa}{Publication 1}
    \bibliographystylepa{bibstyle/myIEEEtran}
        
    \newcites{pb}{Publication 2}
    \bibliographystylepb{bibstyle/myIEEEtran}
        
    \newcites{pc}{Publication 3}
    \bibliographystylepc{bibstyle/myIEEEtran}
    
    \newcites{pd}{Publication 4}
    \bibliographystylepd{bibstyle/myIEEEtran}
    
    \newcites{pe}{Publication 5}
    \bibliographystylepe{bibstyle/myIEEEtran}
    
    \newcites{pf}{Publication 6}
    \bibliographystylepf{bibstyle/myIEEEtran}

    \newcites{pg}{Publication 7}
    \bibliographystylepg{bibstyle/myIEEEtran}

    \newcites{ph}{Publication 8}
    \bibliographystyleph{bibstyle/myIEEEtran}

    %\newcites{pi}{Publication 9}
    %\bibliographystylepi{bibstyle/myIEEEtran}

    %\newcites{pj}{Publication 10}
    %\bibliographystylepj{bibstyle/myIEEEtran}

    % If you have more included publications, then add additional styles here
    
\fi


% For making tabbed pages for included papers
\usepackage{fancytabs} % for adding tabs to pages
% configure the tabs
...
\makeatletter
% define these to avoid complaints about the command being redefined
...
\newcommand{\makeallother}{%
  \gqmcount0\relax
  \loop\ifnum\gqmcount<255\relax
  \catcode\gqmcount12\relax
  \advance\gqmcount by 1\relax
  \repeat
}

\newcommand{\mywritebibfileinfo}[1]{%
  % #1: bib filename
  \IfFileExists{#1}%
  {%
    \begingroup
    ...
    \makeatletter
    \everyeof{\noexpand}%
    \edef\filecontent{\@@input #1 }% Read the entire file into \filecontent
    \immediate\write\citedtagsfile{\filecontent}% Write the macro content
    \endgroup
  }%
  {\wlog{kththesis: missing file: #1}}%
}

% define my own version of the fancytab macro
% This creates black rectangles with white lettering
\newcommand{\myfancytab}[5][RIGHT]{%
  % #1: Alignment (RIGHT)
  % #2: Label/Tag (\ref*{paper:A})
  % #3: Divider Number
    \ifstrequal{#1}{LEFT}
    {
        \renewcommand{\eastwest}{west}
        \renewcommand{\multiplier}{0.5}
    }
    { %else
        \renewcommand{\eastwest}{east}
        \renewcommand{\multiplier}{-0.5}
    }
    ...
    \tikz\draw [fill=black,black]
    (0,0) rectangle (\the\tab@width,\the\tab@height)
    node[rotate=0]
     at (\tab@texthpos*\the\tab@width,\tab@textvpos*\the\tab@height)%
    {\tab@style\textcolor{white}{#2}};%
    };%
    \end{tikzpicture}%
    % Extract the tag from \ref* and the bib file name (you'll need to adapt this)
    \begingroup%
    \ifbiblatex\relax%
    \else%
    \immediate\write\citedtagsfile{@misc{fakebib#3, cited="#4" bib="#5"}} % Write fake bib entry
    \fi%
    \ifstrempty{#5}{}{%   If #5 is empty ignore it
        \mywritebibfileinfo{#5.bib}%
    }%
    \ifbiblatex%
    \noindent\fullcite{#4}%
    \else%
    \ifnum #3=1%
        \nocitepa{#4_pub}\bibliographypa{#5}%
    \fi%
    \ifnum #3=2%
        \nocitepb{#4_pub}\bibliographypb{#5}%
    \fi%
    \ifnum #3=3%
        \nocitepc{#4_pub}\bibliographypc{#5}%
    \fi%
    \ifnum #3=4%
        \nocitepd{#4_pub}\bibliographypd{#5}%
    \fi%
    \ifnum #3=5%
        \nocitepe{#4_pub}\bibliographype{#5}%
    \fi%
    \ifnum #3=6%
        \nocitepf{#4_pub}\bibliographypf{#5}%
    \fi%
    \ifnum #3=7%
        \nocitepg{#4_pub}\bibliographypg{#5}%
    \fi%
    \ifnum #3=8%
        \nociteph{#4_pub}\bibliographyph{#5}%
    \fi%
    \fi % end of non-biblatex case
  \endgroup%
}
\makeatother
...
% The following line is useful if you are having a problem with Overleaf timing out. This typically means that it is taking a long time to prepare and cache fonts, but as a result, it has used up your time quota. So, uncomment it, recompile, letting it cache them and then you can comment out the line again!
%\end{document}
...
%  --- start of optional Dedication page ---
%\begin{comment}
% such a page has no page number shown
\thispagestyle{empty}
\engExpl{A dedication page is optional - it has no heading. If you do \textbf{not} need a dedication page, add a percent symbol before the \textbackslash begin\{comment\} and 
\textbackslash end\{comment\}. If you do want a dedication page, then add your text below the \textbackslash vspace macro.}
\generalExpl{Keep in mind that your thesis is an archival document that will be accessible (ideally) for all time. Therefore, you should only say what you would want others to read in 1, 5, 10, \ldots\ years.}
% set the font to a serif font 12pt in italic typeface
{\rmfamily\fontsize{12}{14}\fontshape{it}\selectfont
\null
\vspace{178pt}
To xxxxx.
}
\clearpage
% Avoid having a page number on the next (verso) page
\thispagestyle{empty}
% advance to an odd page
\cleardoublepage
%\end{comment}
...
%%% entry of the metadata for your thesis.
...
\generalExpl{An abstract should stand on its own, \ie no citations, cross-references to the body of the document, acronyms must be spelled out, \ldots\, .}
\engExpl{Write a draft abstract \textit{early} and \textit{revise as necessary}. This will help keep you focused on what you are trying to do.}
...
\begin{itemize}
  \item What is the topic area? (optional) Introduces the subject area for your research.
  ...
  \item Results/Conclusions/Consequences/Impact: What are your key results/\linebreak[4]conclusions? What will others do based on your results? What can be done now that you have finished - that could not be done before your research was completed?
\end{itemize}
...
LaTeX comments starting with the percent symbol (\%) to the end of the line will be removed.
Note that one can include percentages, such as: 51\% or \qty{51}{\percent}.
...
\warningExpl{A helpful script for those who are comfortable with Python}
For assistance in identifying suitable keywords, a dedicated Python utility, \texttt{thesis\_keyword\_extractor.py}, has been developed. This tool processes the main textual content of the thesis (as a PDF input) and generates a list of potential keywords and specialized terminology. Crucially, it attempts to assign a linguistic proficiency level to each term, based on the Common European Framework of Reference for Languages (CEFR) scale. This analysis is intended to assist the author in selecting precise and appropriately specialized keywords for the thesis.
\footnote{The source code for the keyword extractor and supporting files can be found at \url{https://github.com/gqmaguirejr/Canvas-tools}.}
...
\warningExpl{Inside the following \texttt{scontents} environment, you cannot use a \textbackslash include{filename}, as the command itself, rather than the file's contents, will end up in the for DiVA information. Additionally, you should not use a straight double quote character in the abstracts or keywords, but rather use two single quote characters instead.}
...
\engExpl{If you are writing your thesis in English, you can leave this until the draft version that goes to your internal reviewer. In this way, you can provide the English and Swedish abstract/summary information that can be used in the announcement for your oral defense.\\If you are writing your thesis in English, then this section can be a summary targeted at a more general reader. However, if you are writing your thesis in Swedish, then the reverse is true – your abstract should be for your target audience, while an English summary can be written targeted at a more general audience.\\This means that the English abstract and Swedish \foreignlanguage{swedish}{sammnfattning}  
or Swedish abstract and English summary need not be literal translations of each other.}

\engExpl{Do not use the \texttt{\textbackslash glspl\{\}} command in an abstract that is not in English, as my programs do not know how to generate plurals in other languages. Instead, you will need to spell these terms out or give the proper plural form.}
\engExpl{In fact, it is a good idea to \textbf{not use} the glossary commands at all in an abstract/summary in a language other than the language used in the \texttt{acronyms.tex file} - unless one is going to define a new \texttt{\textbackslash glsaddkey} for the long and short Swedish (or other languages that you are using) versions of acronyms.}
\warningExpl{An example of this might be shown in a future version of this template.}
...
%% If you have not set includeExtraAbstracts in the documentclass,
%% compiling will print a page about including extra abstracts.
%% If you include this option, but do not enable an additional abstract, the
%% page about adding extra abstracts will _not_ be generated.
\ifincludeExtraAbstracts\relax
\else
% Turn off the Swedish running heading
\markboth{Information about additional abstracts}{}
\engExpl{If you add the class option \texttt{includeExtraAbstracts} in your \texttt{documentclass}, then you can enable abstracts/summaries in a number of languages. Include those versions of abstracts that you need.}
...
\warningExpl{Note that you may need to augment the set of languages used in
...
% --- Include Albanian abstract ---
\ifenabledalbanianabstract
\input{Additional_Abstracts/abstract_albanian}
\fi

% --- Include Arabic abstract ---
\ifenabledarabicabstract
\input{Additional_Abstracts/abstract_arabic}
\fi

% --- Include Bosnian abstract ---
\ifenabledbosnianabstract
\input{Additional_Abstracts/abstract_bosnian}
\fi

% --- Include Catalan abstract ---
\ifenabledcatalanabstract
\input{Additional_Abstracts/abstract_catalan}
\fi

% --- Include Central Kurdish (Sorani) abstract ---
\ifenabledcentralkurdishabstract
\input{Additional_Abstracts/abstract_centralkurdish}
\fi

% --- Include Chinese abstract ---
\ifenabledchinesesimplifiedabstract
\input{Additional_Abstracts/abstract_chinese}
\fi

% --- Include Danish abstract ---
\ifenableddanishabstract
\input{Additional_Abstracts/abstract_danish}
\fi

% --- Include Dutch abstract ---
\ifenableddutchabstract
\input{Additional_Abstracts/abstract_dutch}
\fi

% --- Include Estonian abstract ---
\ifenabledestonianabstract
\input{Additional_Abstracts/abstract_estonian}
\fi

% --- Include Finnish abstract ---
\ifenabledfinnishabstract
\input{Additional_Abstracts/abstract_finnish} 
\fi

% --- Include French abstract ---
\ifenabledfrenchabstract
\input{Additional_Abstracts/abstract_french}
\fi

% --- Include German abstract ---
\ifenabledgermanabstract
\input{Additional_Abstracts/abstract_german}
\fi

% --- Include Greek abstract ---
\ifenabledgreekabstract
\input{Additional_Abstracts/abstract_greek}
\fi

% --- Include Hebrew abstract ---
\ifenabledhebrewabstract
\input{Additional_Abstracts/abstract_hebrew}
\fi

% --- Include Hindi abstract ---
\ifenabledhindiabstract
\input{Additional_Abstracts/abstract_hindi}
\fi

% --- Include Hungarian abstract ---
\ifenabledhungarianabstract
\input{Additional_Abstracts/abstract_hungarian}
\fi

% --- Include Icelandic abstract ---
\ifenabledicelandicabstract
\input{Additional_Abstracts/abstract_icelandic}
\fi

% --- Include Italian abstract ---
\ifenableditalianabstract
\input{Additional_Abstracts/abstract_italian}
\fi

% --- Include Japanese abstract ---
\ifenabledjapaneseabstract
\input{Additional_Abstracts/abstract_japanese}
\fi

% --- Include Latin abstract ---
\ifenabledlatinabstract
\input{Additional_Abstracts/abstract_latin}
\fi

% --- Include Latvian abstract ---
\ifenabledlatvianabstract
\input{Additional_Abstracts/abstract_latvian}
\fi

% --- Include Meänkieli abstract ---
\ifenabledmeänkieliabstract
\input{Additional_Abstracts/abstract_meänkieli}
\fi

% --- Include Davvisámegiela (northernsami, nordsamiska) abstract ---
\ifenablednorthernsamiabstract
\input{Additional_Abstracts/abstract_northernsami}
\fi

% --- Include Norwegian abstract ---
\ifenablednorwegianabstract
\input{Additional_Abstracts/abstract_norweign}
\fi

% --- Include Polish abstract ---
\ifenabledpolishabstract
\input{Additional_Abstracts/abstract_polish}
\fi

% --- Include Portuguese abstract ---
\ifenabledportugueseabstract
\input{Additional_Abstracts/abstract_portuguese}
\fi

% --- Include Romanian abstract ---
\ifenabledromanianabstract
\input{Additional_Abstracts/abstract_romanian}
\fi

% --- Include Russian summary ---
\ifenabledrussianabstract
\input{Additional_Abstracts/abstract_russian}
\fi

% --- Include Serbian abstract ---
\ifenabledserbianabstract
\input{Additional_Abstracts/abstract_serbian}
\fi

% --- Include Slovak abstract ---
\ifenabledslovakabstract
\input{Additional_Abstracts/abstract_slovak}
\fi

% --- Include South Sami (sydsamiska (södra)) abstract ---
\ifenabledsouthsamiabstract
\input{Additional_Abstracts/abstract_southsami}
\fi

% --- Include Spanish abstract ---
\ifenabledspanishabstract
\input{Additional_Abstracts/abstract_spanish}
\fi

% --- Include Swahili / Kiswahili summary ---
\ifenabledswahiliabstract
\input{Additional_Abstracts/abstract_swahili}
\fi

% --- Include Turkish abstract ---
\ifenabledturkishabstract
\input{Additional_Abstracts/abstract_turkish}
\fi

% --- Include Ukrainian abstract ---
\ifenabledukrainianabstract
\input{Additional_Abstracts/abstract_ukrainian}
\fi

% --- Include Vietnamese abstract ---
\ifenabledvietnameseabstract
\input{Additional_Abstracts/abstract_vietnamese}
\fi

% --- Include Yiddish abstract ---
\ifenabledyiddishabstract
\input{Additional_Abstracts/abstract_yiddish}
\fi

\fi % end of ifincludeExtraAbstracts
...
% --- include (optional) Acknowledgments ---
\chapter*{Acknowledgments}
\markboth{Acknowledgments}{}
\sweExpl{Författarnas tack}

\engExpl{It is nice to acknowledge the people who have helped you. It is
...
\engExpl{Be sure that you acknowledge your funding source(s).}
\sweExpl{Se till att du anger din/dina finansieringskällor.}
...
% Select the font used for the TOC heading
\renewcommand{\cfttoctitlefont}{\FigtreeFont\fontsize{24}{26}\fontseries{md}\selectfont}  %%% optionally add at the end \MakeUppercase
\tableofcontents
\markboth{\contentsname}{}
\cleardoublepage

% Include List of Figures (LOF) ---
\renewcommand{\cftloftitlefont}{\FigtreeFont\fontsize{24}{26}\fontseries{md}\selectfont} 
\listoffigures
\cleardoublepage

% Include List of Table (LOT) ---
\renewcommand{\cftlottitlefont}{\FigtreeFont\fontsize{24}{26}\fontseries{md}\selectfont} 
\listoftables
\cleardoublepage
\lstlistoflistings\engExpl{If you have listings in your thesis. If not, then remove this preface page.}
\cleardoublepage

% Include List of Algorithms (LOA) ---
%\listofalgorithms\engExpl{If you have many algorithms in your thesis. If not, then remove this preface page.}
%\cleardoublepage
...
% Alter the formatting of the title
% {{\vspace*{-80pt}\fontsize{19}{22}\selectfont\glossaryname\vspace*{-40pt}}}
\ifinswedish
...
% List of Publications - If you have included publications -- 
\ifincludepublications
    % In the case of a compilation thesis, i.e., with Kappa + papers
    % It is convenient to have a list of papers that are included in the thesis
    \chapter*{List of Publications}
    \label{chap:listOfPublications}

    \engExpl{When defending a thesis, you must mention which scientific papers, patents, artifacts, datasets, … are included. Note that peer-reviewed artifacts are increasingly common, as some publications have ``badges'' for publications that include code, code that has been shown by a reviewer to be reproducible, \etc. See, for example, Artifact A.}
    \generalExpl{The titles of the publications are to be set in Figtree; however, this may not be possible, as Figtree lacks Greek characters, math symbols, \etc., that may appear in the titles. Therefore, you may need to change the font you are using for such a title. Papers E and F show two approaches for doing this. Paper E uses math mode to get the Greek characters, while Paper F uses a font that has the Greek characters. Further note that \textbackslash mystixmathfont has been used, rather than a font such as \textbackslash NotoSansFont - as the overbar in the latter font is too low. The font could have been applied to the whole title, but this would result in serifed letters for the whole title. Both of these commands for fonts were introduced in \texttt{kththesis.cls}.}
    \warningExpl{The use of math mode in the title of Paper E will generate a warning "Font shape `U/stmry/m/n' in size <5.5> not available" - since scaling the 11pt math font for the superscripts and subscripts leads to 5.5pt letters - for which LaTeX substitutes 5pt letters. Both for this font scaling issue and to improve readability (see, for example, the fictitious title of Report A), I would think that the publication titles should be set in a font larger than 11pt.}
    % alternatively use \small as the size - this may be suitable if you have titles that contain characters not in Figtree
    {\FigtreeFont\fontsize{11}{13}\fontshape{n}\selectfont
% Check if there was an automatically generated list of publications file.
\IfFileExists{lib/publications\_generated}{
      \input{lib/publications_generated}
}{
    \begin{ListOfPapers}
        \item \label{paper:A} Coherent {File} {Distribution} {Protocol}\cite{ioannidis_coherent_1991}
        \item \label{paper:B} A {New} {Automated} {Way} to {Measure}{Polyethylene} {Wear} in {THA} {Using} a {High} {Resolution} {CT} {Scanner}: {Method} and {Analysis}\,\cite{maguire_jr_new_2014}
    ...
    \ifbiblatex
        \item \label{paper:E} \citefield{verardo2023fmmheadenhancingautoencoderbasedecg}{title}
   \else
    ...
    \begin{ListOfArtifacts}
        \item \label{artifact:A} PacketMill: Toward Per-Core 100-Gbps
Networking - Artifact for ASPLOS’21\,\cite{10.5281/zenodo.4435970}
    \end{ListOfArtifacts}
    } % end of font application
}
    \engExpl{Those who want can also list other peer-reviewed original scientific publications not included in the thesis. I would include granted patents (relevant to the thesis) in this list.}
    \warningExpl{If you are using BibLaTeX, \ref{paper:F} shows an example of how you can easily include the title by simply citing the title field of a reference. The alternative uses math mode to set the special characters.\\In contrast, \ref{paper:G} shows how you can use the unicode version of the characters and explicitly control the font used to set them.}
    \section*{Scientific papers not included in the thesis}
    {\FigtreeFont\fontsize{11}{13}\fontshape{n}\selectfont
    \begin{ListOfPapers}
    \ifbiblatex
        \item \label{paper:F} \citefield{Noz_1975}{title}
    \else
        \item \label{paper:F}  {{Calculation of $\bar{E}_{\beta}$, $\Gamma$ and $\Delta_{i}$ for $^{99m}$Tc}}
    \fi
        \item \label{paper:G} Calculation of {\mystixmathfont \={Ε}\textsubscript{β}}, {\mystixmathfont Γ} and {\mystixmathfont Δ\textsubscript{i}} for \textsuperscript{99m}Tc

    \end{ListOfPapers}

    \begin{ListOfPatents}
        \item \label{patent:C} Title of patent
        \item \label{patent:D} Title of patent
    \end{ListOfPatents}
    } % end of font application

    \engExpl{Those who want can also list other patent applications/posters/reports/artifacts/... not included in the thesis.}
    \section*{Patent applications/posters/$\ldots$ not included in the thesis}
    {\FigtreeFont\fontsize{11}{13}\fontshape{n}\selectfont
    \begin{ListOfPatentApplications}
        \item \label{patentApplication:A} Title of patent application
        \item \label{patentApplication:B} Title of patent application
    \end{ListOfPatentApplications}

    \begin{ListOfPosters}
        \item \label{poster:A} Title of poster
        \item \label{poster:B} Title of poster
    \end{ListOfPosters}

    \begin{ListOfReports}
        \item \label{report:A} Comparing Bose-Einstein statistics $\Gamma^{B-E}_{HH\mapsto SS}$ to Maxwell-Boltzmann statistics $\Gamma^{M-B}_{HH\mapsto SS}$ in the context of Feebly Interacting Massive particles % Inspired by diva2:1577999
        \item \label{report:B} Title of report
    \end{ListOfReports}

    \begin{ListOfArtifacts}
        \item \label{artifact:B} Title of artifact
        \item \label{artifact:C} Title of artifact
    \end{ListOfArtifacts}
    
    \begin{ListOfDatasets}
        \item \label{dataset:A} Title of dataset
    \end{ListOfDatasets}
    } % end of font application

\fi % end of ifincludepublications

% --- End of preface pages ---
%% The following label is essential to know the page number of the last page of the preface
%% It is used to compute the data for the "For DIVA" pages
% The phantom section ensures there is a "section" at this point that the label can refer to

% --- Place the label here, at the end of the content ---
\phantomsection
\label{pg:lastPageofPreface}
% ----------------------------------------------------

% This command will end the preface and start the main document
\cleardoublepage

% Start of main matter - i.e., the body of the thesis itself ---
% Mainmatter is where the actual contents of the thesis goes
\mainmatter

%% For a thesis with included publications - mark the start of the Kappa
\ifincludepublications
    \cleardoublepage
    % Turn off fancy headings for dividers and included pages
    \fancyhead{}  % Do not use header on this extra page or pages

    % Set a page for this part of the thesis
    \part{Kappa}
    \cleardoublepage
\fi % end of includepublications
...
goals of this thesis, and outlines the structure of the thesis.\\
...
use the command \texttt{parencite} to get a reference in parentheses or square brackets, like
this \textbackslash parencite\{ioannidis\_coherent\_1991\}, resulting in \parencite{ioannidis_coherent_1991}.  It is also possible to include the author as part of the sentence using \texttt{textcite}, when talking about the work of \textbackslash textcite\{maguire\_jr\_new\_2014\} resulting in \textcite{maguire_jr_new_2014}.\\
...
cache performance in \parencite{farshin_make_2019} in the context of links running
at \qty{200}{Gbps}.}
\fi
...
\generalExpl{Present the background for the area. Set the context for your research – so that your reader can understand both your research and this thesis. (Give detailed background information in \Cref{ch:background} - together with related work.)
...
Clearly state the research questions your thesis addresses.

\begin{enumerate}[leftmargin=*, label=\textbf{RQ \arabic*}, ref=RQ \arabic*] %labelindent=1em for indent
    \itemsep0em
    \item \label{x:rq1} research question 1,
    \item \label{x:rq2} research question 2,
    \item \label{x:rq3} research question 3, and
    \item \label{x:rq4} research question 4.
\end{enumerate}
...
\section{Purpose}
\sweExpl{Syfte}
\sweExpl{Skilj på syfte och mål! Syfte är att förändra något till det bättre. I avhandling finns ofta två aspekter på detta. Dels vill problemägaren (företaget) få sitt problem löst till det bättre men akademin och ingenjörssamfundet vill också få nya erfarenheter och vetskap. Beskriv ett syfte som tillfredställer båda dessa aspekter.\\
Det finns även ett syfte till som kan vara värt att beakta och det är att du som student skall ta examen och att du måste bevisa, i ditt avhandling, att du uppfyller examensmålen. %Dessa mål sammanfaller med kursmålen för examensarbetskursen. 
}
\generalExpl{State the purpose of your thesis and the purpose of your research.\\
Describe who benefits and how they benefit if you achieve your goals. Include anticipated ethical, sustainability, social issues, \etc related to your research. (Return to these in your reflections in \Cref{sec:reflections}.)}
...
\generalExpl{State the goal/goals of your research.}

The goal of this research is XXX. This has been divided into the following three sub-goals:
\begin{enumerate}
...
\generalExpl{In addition to presenting the goal(s), you might also state what the deliverables and results of the research are.}

\generalExpl{The goals should relate to the knowledge gaps and research questions that you identified in \Cref{sec:problem}. If you have written a compilation thesis, couple your description to your included publications.}
...
Tänk på att företaget ibland måste stå tillbaka i sin önskan och förväntan på projektets resultat till förmån för ny eller kompletterande ingenjörserfarenhet och vetenskap (ditt avhandling). Det är du som student som bestämmer och löser fördelningen mellan dessa två intressen men se till att alla är informerade. }
\generalExpl{Introduce your choice of methodology/methodologies and method/methods – and the reason why you chose them. Contrast them with and explain why you did not choose other methodologies or methods. (The details of the actual methodology and method you have chosen will be given in \Cref{ch:methods}. Note that in \Cref{ch:methods}, the focus could be research strategies, data collection, data analysis, and quality assurance.)\\
In this section you should present your philosophical assumption(s), research method(s), and research approach(es).}

\section{Delimitations}\sweExpl{Avgränsningar}
\generalExpl{Describe the boundary/limits of your thesis and what you are explicitly not going to do. This will help you bound your efforts – as you have clearly defined what is out of the scope of this thesis. Explain the delimitations. These are all the things that could affect the study if they were examined and included in your thesis.}

\section{Thesis contributions}\sweExpl{Avhandlingsbidrag}
How does your thesis contribute to addressing the research questions?

If you have done this work with others, clearly state what your own contributions were. If you have written a compilation thesis, give this description for each of your included publications. You might use the taxonomy defined in \gls{CRediT}\cite{brand_beyond_2015}\footnote{See \url{https://authorservices.wiley.com/author-resources/Journal-Authors/open-access/credit.html}}.
%%% The roles shown in the template are simply to illustrate what is possible and were not designed to be definitive.
\IfFileExists{lib/thesis_contributions_generated.tex}{\input{lib/thesis_contributions_generated.tex}}

\section{Structure of the thesis}\sweExpl{Rapportens disposition}
\Cref{ch:background} presents relevant background information about xxx.  \Cref{ch:methods} presents the methodology and method used to solve the problem. 
...
how prior work/prior research will be applied on or used in your
research (described in this thesis). Explain why and what is not used in
your research and give valid reasons for rejecting the work/research.}
...
There are xxx characteristics that distinguish yyy from other \gls{ICT} systems, as shown in \Cref{fig:lotsofstars}. \Cref{tab:tablecaracteristics} summarizes these characteristics.
...
\end{figure}
\sweExpl{Massor av stjärnor (Inspirerad av figur x.y på sidan z i [xxx])}
...
See \Cref{fig:ieee8023-data-packet} which uses the \textsf{bytefield}  \LaTeX\ package. 
...
an IPv4 packet is shown in \Cref{fig:ipv4-header}. This should be
contrasted with the IPv6 header shown in \Cref{fig:ipv6-header}.
...
Carrier clouds have been proposed as a means to reduce the delay between users and the cloud server that provides them with content. However, there is a question of how to find the available resources in such a carrier cloud. One approach has been to disseminate resource information using an extension to OSPF-TE, see Roozbeh, Sefidcon, and Maguire \cite{roozbeh_resource_2013}.
...
\engExpl{It is nice to have this chapter conclude with a summary. For
  ...
  benefits and drawbacks with each -- so that later you can compare your solution
...
\generalExpl{Describe the engineering-related contents (preferably with models) and the research methodology and methods that are used in your research.\\
...
used in this thesis. \Cref{sec:researchProcess} describes the research
process. \Cref{sec:researchParadigm} details the research
paradigm. \Cref{sec:dataCollection} focuses on the data collection
techniques used for this research. \Cref{sec:experimentalDesign}
describes the experimental design. \Cref{sec:assessingReliability}
explains the techniques used to evaluate the reliability and validity of the
data collected. \Cref{sec:plannedDataAnalysis} describes the method
used for the data analysis. Finally, \Cref{sec:evaluationFramework}
...
(4) Vad behöver du som student leverera för att uppnå hög kvalitet (minimikrav) eller mycket hög kvalitet på avhandling?\\
...
\sweExpl{Undersökningsprocess och utvecklingsprocess}

\Cref{fig:researchprocess} shows the steps conducted to carry out this research. 
...
\section[Hardware/Software design …/Model/Simulation model and parameters/…]{Hardware/Software design …/Model/Simulation model \& parameters/…}
\sweExpl{Hårdvara / Mjukvarudesign ... / modell / Simuleringsmodell och parametrar / …}

\Cref{fig:homepageicon} shows a simple icon for a home page. The time
to access this page when served will be quantified in a series of
experiments. The configurations that have been tested in the test bed are
listed in \Cref{tab:configstested}. In \qty{7.0}{\percent} of cases, there was an error indicating xxxxx.
...
\Cref{tab:configstested} shows an example of how one can automatically resize a table so that it fills the column width.
...
\begin{table}[!ht]
  \begin{center}
    ...
    \begin{tabular}{l|c} % <-- Alignments: 1st column left, 2nd middle and 3rd right, with vertical lines in between
      ...
      2 & Simple test with two servers\\
    \end{tabular}
    }
  \end{center}
\end{table}
...
\sweExpl{Det här avsnittet är helt enkelt för att visa ett exempel på hur du kan inkludera kod i ditt avhandling - det här är inte ett avsnitt du skulle ha i ditt avhandling.}


\Needspace*{8\baselineskip}
\Cref{lst:helloWorldInC} shows an example of a simple program written
in C code.
...
\engExpl{This template uses the package \texttt{lstlistings} for many different listings. Alternatively, one could use the \texttt{minited} package together with the \texttt{listings} environment, see, for example, \href{https://www.overleaf.com/learn/latex/Code_Highlighting_with_minted}{Code Highlighting with minted} and \Cref{lst:exampleUsingMinted}. }

In contrast, \cref{lst:programmes} is an example of code in Python to
get a list of all of the programs at KTH. Note that starting on {\DTMsetdatestyle{iso}\DTMdisplaydate{2025}{6}{1}{-1}} the KOPPS API will no longer work.



\engExpl{Note that the change to the iso date format in the line above was done in a group so that afterwards the date style returns to what it was, resulting in 
\DTMdisplaydate{2025}{6}{1}{-1}.}
...
def make_spreadsheet_of_differences(pds):
    ...
    return publishers
\end{minted}
\caption[Using minted with python code]{Using \texttt{minted} with python code}
\label{lst:exampleUsingMinted}
\end{listing}

\noindent In \Cref{lst:exampleUsingMinted}, we create a \texttt{ExcelWriter} function and use it to write a panda dataframe (named \texttt{comp}) to the spreadsheet. Finally, \texttt{writer.close()} closes the open spreadsheet.
\FloatBarrier
...
\sweExpl{Det här avsnittet är helt enkelt för att visa ett exempel på hur du kan rita dina egna figurer i ditt avhandling – det här är inte ett avsnitt du skulle ha i ditt avhandling.}
...
\Cref{fig:vmsVersusContainers} shows a comparison of virtual machines (VMs) versus containers.
...
Some statistics of the delay measurements are shown in \Cref{tab:delayMeasurements}.
...
\Cref{tab:ping_results} shows the measurement of round-trip times from four hosts to and from a server.
\begin{table}[ht!]
...
\Cref{fig:processing_vs_payload_length} shows an example of the performance as measured in the experiments.
...
(percentage)]{Rust types distribution for the compiler, crates.io, and lib.rs.
...
\sweExpl{Slutsatser}
\engExpl{Describe the conclusions (reflect on the whole introduction given in \Cref{ch:introduction}).}


  
\engExpl{Discuss the positive effects and the drawbacks.\\
Describe the evaluation of the results of your research.\\
...
met. In these sections, we will focus on some of the remaining issues that
should be addressed in future work. ...

\subsection{What has been left undone?}
\label{sec:what-has-been-left-undone}

The prototype does not address the third requirement, \ie a yearly unavailability of less than 3 minutes; this remains an open problem. ...
...
\engExpl{In the references, let Zotero or other tool fill this in for you. I suggest an extended version of the IEEE style, to include URLs, DOIs, ISBNs, \etc., to make it easier for your reader to find them. This will make life easier for your opponent, grading committee, and readers. \\IEEE Editorial Style Manual: \url{https://www.ieee.org/content/dam/ieee-org/ieee/web/org/conferences/style_references_manual.pdf}}
\sweExpl{Låt Zotero eller annat verktyg fylla i det här för dig. Jag föreslår en utökad version av IEEE stil - att inkludera webbadresser, DOI, ISBN osv. - för att göra det lättare för läsaren att hitta dem. Detta kommer att göra livet lättare för dina opponent, betygsnämnd och läsarna.}
...
% --- Include appendices as necessary --- 
...
\section{Directly: Attach files to the PDF file}
...
The BibTeX references used in this thesis are attached.
    \IfFileExists{references_used.bib}{
    \attachfile[description={references_used.bib}]{references_used.bib}}
    {\attachfile[description={references.bib}]{references.bib}}
\engExpl{Here we have used \texttt{\textbackslash attachfile} and attached the file. There is also a mechanism for \textbf{embedding} files into PDF, but I do not use it or describe it further here -- for reasons of \first simplicity, \Second \texttt{\textbackslash attachfile} supports any type of file, \third the attached files are visible as annotations on the page where they are included, and  \fourth potential compatibility with PDF/A-3.}

Your reader can access the attached files using a PDF tool such as Adobe Acrobat Reader using the paperclip icon in the left menu, as shown in \Cref{fig:PDFreaderPaperclipExample} or by right-clicking on the push-pin icon in the PDF file and then using the menu to save the embedded file as shown in \Cref{fig:PDFreaderPushpinExample}.

An argument for including supporting material in the PDF file is that it will be available to anyone who has a copy of the PDF file. As a result, they do not have to look elsewhere for this material. This comes at the cost of a larger PDF file. However, the attached files are encoded into a compressed stream within the PDF file; thus, reducing the number of additional bytes. For example, the references.bib file that was used in this example is \qty{18093}{\byte} in size but only occupies \qty{7087}{\byte} in the PDF file.

\engExpl{Note that the figures are for an earlier version of the \texttt{references.bib} file; hence, the size does not match that stated in the above paragraph.}
...
\section{Indirectly: Using a URL}
% Attach source code file(s) or add a URL to the github or other repository
Some source code relevant to this thesis can be found at \url{https://github.com/gqmaguirejr/E-learning} and \url{https://github.com/gqmaguirejr/Canvas-tools}.

If you used a research data repository, you should include a URL to your data (at some point in the thesis or, at worst, in an appendix such as this). Some potential data repositories are: \foreignlanguage{swedish}{Svensk nationell datatjänst} (\href{https://snd.se/sv}{SND}) and \href{https://researchdata.se/}{Researchdata.se}; \foreignlanguage{swedish}{Sveriges dataportal} (\href{https://www.digg.se/}{DIGG}); \href{https://zenodo.org/}{Zenodo} and
\href{https://zenodo.org/communities/eu/}{EU Open Research Repository}; and \href{https://figshare.com/}{Figshare}.

\section{Uploading supporting materials into DiVA}

\warningExpl{DiVA is limited to $\approx$\qty{1}{\giga\byte} for each supporting file. If you have very large amounts of supporting material, you will probably want to use one of the data repositories. For additional help with this, contact KTH Library via 
...
% --- Include nomenclature if needed ---
% Include an example of using nomenclature
\ifnomenclature
    \cleardoublepage
    \chapter{Main equations}
    \label{ch:NomenclatureExamples}
    This appendix provides examples of equations used throughout this thesis.
    ...
    \nomenclature{$m$}{The mass of one angel} follows easily from \Cref{eq:mainEq}.

    \section{An even simpler example}
    The formula for the diameter of a circle is shown in \Cref{eq:secondEq} while the formula for the area of a circle is shown in \cref{eq:thirdEq}.
...
\cleardoublepage
\fi  %% end of nomenclature



%% for thesis with included publications
\begin{comment}
\ifincludepublications
    \cleardoublepage
    % Turn off fancy headings for dividers and included pages
...
% For each included publication, create the fancy tab
% First publication
\thispagestyle{empty}
% Use an environment to format the information about this publication
\begin{dividerContent}{0cm}{5cm}
\myfancytab[RIGHT]{\ref*{paper:A}}{1}{ioannidis_coherent_1991}{Included_publications/Paper_A}
...
\thispagestyle{empty}
\begin{dividerContent}{0cm}{5cm}
\myfancytab[RIGHT]{\ref*{patent:A}}{6}{US7107453B2}{Included_publications/Patent_A}


This publication is a U.S. Government publication; hence, it has no copyright.
\end{dividerContent}
\cleardoublepage
%% Include US12111768.pdf
%% can include as options pagecommand={\thispagestyle{fancy}},frame to keep pagination and running headings
% Just include 2 pages for this example
\includepdf[pages={1-2},scale=0.9]{Included_publications/US7107453.pdf}

% Seventh publication
\thispagestyle{empty}\begin{dividerContent}{0cm}{5cm}
\myfancytab[RIGHT]{\ref*{patent:B}}{7}{US12111768B2}{Included_publications/Patent_B}

This publication is a U.S. Government publication; hence, it has no copyright.
\end{dividerContent}

\cleardoublepage
%% Include US7107453.pdf
%% can include as options pagecommand={\thispagestyle{fancy}},frame to keep pagination and running headings
% Just include 2 pages for this example
...
\thispagestyle{empty}
\begin{dividerContent}{0cm}{5cm}
\myfancytab[RIGHT]{\ref*{artifact:A}}{8}{10.5281/zenodo.4435970}{Included_publications/Artifact_A}
...
\FileClose{citedtagsfile} % close the file that the bib entries are being written to

\fi % end the ifincludepublications conditional 
\end{comment}
% input the automatically generated dividers and papers
\IfFileExists{lib/publications_dividers_generated.tex}{\input{lib/publications_dividers_generated.tex}}
...
% --- Include the metadata for DiVA ----
\divainfo
\fancyhead{}  % Do not use header on this extra page or pages
\thispagestyle{empty}
\section*{€€€€ For DIVA €€€€}
\begin{NoHyper} % turn off hyperlinks
    \sffamily\tiny
    \begin{flushleft}
%\lstset{numbers=none} %% remove any list line numbering

\IfFileExists{fordiva.json}{
\inputminted[breaklines]{json}{fordiva.json}
}
{}
    ...
    % Attach the citedtags.bib file
    \IfFileExists{citedtags.bib}{
    \attachfile[description={cited tags bib file}]{citedtags.bib}
    }{}

    % If there is an acronyms.tex file, add it
    \IfFileExists{lib/acronyms.tex}{
 \attachfile[description={acronyms.tex}]{lib/acronyms.tex}
    }{}
